...
from objects import flaggedWords
...
if __name__ == "__main__":
	...
	try:
		...
		try:
			consoleHelper.printNoNl("> Loading flagged words... ")
			glob.flaggedWords = flaggedWords.flaggedWords()
			consoleHelper.printDone()
		except:
			consoleHelper.printError()
			consoleHelper.printColored("[!] Error while loading flagged words. Make sure there is a flaggedwords.txt file present", bcolors.RED)
			raise
		...
		#todo: send restart packet when server is shut down
		system.dispose()
