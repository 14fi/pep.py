...
def faq(fro, chan, message):
	...
	messages = {
		"rules": "Please make sure to check (osuHOW rules)[https://osuhow.cf/doc/rules].",
		...
		"discord": "(Join osuHOW Discord!)[https://discord.gg/YPgcJnntWw]",
...
def moderated(fro, chan, message):
	try:
		...
		return "You cannot use moderated mode in a private chat."
...
def fokabotReconnect(fro, chan, message):
	...
	if glob.tokens.getTokenFromUserID(999) is not None:
		return "{} is already connected.".format(glob.BOT_NAME)
...
def ban(fro, chan, message):
	...
	if targetUserID in (999, 1000, 1002):
		...
		userUtils.appendNotes(userID, "Attempted to ban important user.")
		return "Rogue mod detected! Banning."
	# Set allowed to 0
...
def restrict(fro, chan, message):
	...
	if targetUserID in (999, 1000, 1002):
		userUtils.ban(userID)
		userUtils.appendNotes(userID, "Attempted to restrict important user.")
		return "Rogue mod detected! Banning."
...
def freeze(fro, chan, message):
	...
	targetToken = glob.tokens.getTokenFromUsername(userUtils.safeUsername(target), safe=True)
	if targetToken is not None:
		targetToken.enqueue(serverPackets.notification("You have been frozen! The osuHOW staff team has found you suspicious and would like to request a liveplay. Visit osuhow.cf for more info."))
...
def changeUsername(fro, chan, message):
	...
	for i in tokens:
		i.kick("Your username has been changed to {}. Please log out and back in.".format(new))
...
def getPPMessage(userID, just_data = False):
	try:
		...
		msg = "Please note this command is using an older pp algorithm and results are not 100% accurate"
		...
		# API error
		log.error("Error while contacting LETS api.")
...
def report(fro, chan, message):
	...
	try:
		...
		log.warning(adminMsg, discord="ac")
	except exceptions.invalidUserException:
		msg = "u pay me 1 millon dolar maybe i let u report me".format(glob.BOT_NAME)
...
def editMap(fro, chan, message): # Edit maps ranking status ingame. // Added by cmyui and edited by Aoba // shut up aoba you literally do nothing but copy other peoples work
...
commands = [
	{
		...
		"trigger": "!help",
		"response": "Click (here)[https://osuhow.cf/doc/4] for full command list"
	}, {
		"trigger": "!ppboard",
