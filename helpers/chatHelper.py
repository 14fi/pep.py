...
def sendMessage(fro = "", to = "", message = "", token = None, toIRC = True):
	...
	try:
		...
		unfiltered_message = message[:2048]+"..." if len(message) > 2048 else message

		# Check for word filters
		if token.userID > 999:
			message = glob.chatFilters.filterMessage(unfiltered_message)
		else:
		...
		if token.userID > 999:
			# thanks akatsuki for the idea
			# TODO: this seems a bit slow
			webhook = None
			for word in glob.flaggedWords.flaggedWords:
				if word in unfiltered_message:
					if not "\x01ACTION" in unfiltered_message:
						webhook = "ac" 
						break
			if webhook != None:
				log.warning('\n\n'.join([
					f'[{token.username}](https://osuhow.cf/u/{token.userID}) sent a suspicious message',
					unfiltered_message,
					f'Detected word: {word}'
				]), discord=webhook)
